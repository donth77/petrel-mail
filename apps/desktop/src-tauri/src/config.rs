...
///
/// Explicit rather than derived from the IMAP host by string substitution:
/// autoconfig answers both, and a provider like Namecheap uses one host for
/// both while another uses two. The password comes from the keychain, so
/// call this *without* the store lock — same rule as IMAP.
