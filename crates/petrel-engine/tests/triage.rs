...
fn undo_restores_prior_flags_rather_than_inverting_the_action() {
    ...
    let r = store
        .apply_thread_action(account, tid, ActionKind::MarkUnread, None)
        .unwrap();
...
fn an_action_applies_to_every_message_in_the_conversation() {
    ...
    let tid = thread_of(&store, ids[0]);
    let r = store
        .apply_thread_action(account, tid, ActionKind::Star, None)
        .unwrap();
...
fn moving_to_a_named_folder_is_undoable() {
    ...
    let receipt = store
        .apply_thread_action(account, tid, ActionKind::Move, Some(dest))
        .unwrap();
...
fn a_move_without_a_destination_is_refused_rather_than_guessed() {
    ...
    let tid = thread_of(&store, ids[0]);
    let err = store
        .apply_thread_action(account, tid, ActionKind::Move, None)
        .unwrap_err();
...
fn undoing_a_tag_restores_prior_tags_rather_than_inverting() {
    ...
    let receipt = store
        .apply_thread_action(account, tid, ActionKind::Tag, Some(urgent))
        .unwrap();
    store.undo_action(receipt.action_id).unwrap();
    assert_eq!(
...
fn untagging_and_undoing_it_puts_the_tag_back() {
    ...
    let receipt = store
        .apply_thread_action(account, tid, ActionKind::Untag, Some(urgent))
        .unwrap();
