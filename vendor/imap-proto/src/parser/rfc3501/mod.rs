...
mod tests {
    ...
    #[test]
    fn test_envelope_utf8_quoted_strings() {
        // Synthetic UTF-8 ENVELOPE FETCH line — real servers emit 8-bit quoted-strings
        // that strict CHAR rejects (TakeWhile1); addresses are fake example domains only.
        let mut line = format!(
            r#"* 1 FETCH (UID 42 FLAGS () RFC822.SIZE 100 ENVELOPE (NIL "{subject}" (("{name}" NIL "info" "example.jp")) (("{name}" NIL "info" "example.jp")) (("{name}" NIL "info" "example.jp")) ((NIL NIL "user" "example.com")) NIL NIL NIL "<synthetic@example.test>"))"#,
            subject = "会議の件",
            name = "事務局",
        )
        .into_bytes();
        line.extend_from_slice(b"\r\n");
        match super::response_data(&line) {
            Ok((_, Response::Fetch(_, attrs))) => {
                assert!(attrs
                    .iter()
                    .any(|a| matches!(a, AttributeValue::Envelope(_))));
            }
            rsp => panic!("unexpected response {rsp:?}"),
        }
    }
